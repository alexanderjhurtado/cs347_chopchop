...
def get_action_events(volume_arr):
    ...
    slices_in_seconds = [(start_idx*DELTA, end_idx*DELTA) for start_idx, end_idx in event_slices]
    return [(t_start, t_end) for t_start, t_end in slices_in_seconds if t_end-t_start > MIN_CLIP_DURATION]
    ...
    with VideoFileClip(raw_filepath) as clip:
        ...
        with tqdm(total=len(events)) as pbar:
            for idx, (t_start, t_end) in enumerate(events):
                clip_number = str(idx).zfill(len(str(len(events)))) # makes a zero-padded string of the clip index (e.g. '3' -> '003')
                pbar.set_description(f"Saving clip #{clip_number}")
                save_video_clip(clip, f"{CLIP_DIRECTORY}/clip_{clip_number}.MOV", t_start, t_end)
                pbar.update()

if __name__ == "__main__":
    if len(sys.argv) < 2:
        raise Exception("Specify the raw video's filename (e.g. `python3 script.py raw_vid.MOV`)")
